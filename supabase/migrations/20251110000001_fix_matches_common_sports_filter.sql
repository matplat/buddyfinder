-- Fix get_matches_for_user to return only users with common sports
-- This migration replaces the function to add proper filtering
...
BEGIN
  ...
  RETURN QUERY WITH current_user_sports AS (
    -- Get all sports of the current user
    SELECT sport_id
    FROM user_sports
    WHERE user_id = current_user_id
  ),
  matched_profiles AS (
    -- Find profiles within range that have common sports
    SELECT 
      p2.id,
      p2.username,
      p2.display_name,
      ...
      COUNT(DISTINCT CASE 
        WHEN us.sport_id IN (SELECT sport_id FROM current_user_sports) 
        THEN us.sport_id 
      END) as common_sports_count
    FROM profiles p2
    -- Join with auth.users to get email
    LEFT JOIN auth.users au ON au.id = p2.id
    -- Join with user_sports to check for common sports
    INNER JOIN user_sports us ON us.user_id = p2.id
    ...
    WHERE 
      ...
      -- Filter: only users with at least one common sport
      AND us.sport_id IN (SELECT sport_id FROM current_user_sports)
    GROUP BY p2.id, p2.username, p2.display_name, p2.social_links, au.email
    HAVING COUNT(DISTINCT us.sport_id) > 0
  )
  SELECT 
