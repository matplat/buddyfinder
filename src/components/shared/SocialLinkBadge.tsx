...
export const SocialLinkBadge: FC<SocialLinkBadgeProps> = ({ platform, url, onEdit, onDelete }) => {
      ...
      <CardContent className="p-4 flex items-center justify-between">
        ...
        {isEditable && (
          <div className="flex gap-2">
            <Button variant="ghost" size="icon" onClick={handleEdit} aria-label={`Edytuj link do ${name}`}>
              <Pencil className="h-4 w-4" />
            </Button>
            <Button variant="ghost" size="icon" onClick={handleDelete} aria-label={`Usuń link do ${name}`}>
